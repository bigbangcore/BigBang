...
bool CConsensus::GetNextConsensus(CAgreementBlock& consParam)
{
    ...
    int64 nNextBlockTime = pCoreProtocol->GetNextBlockTimeStamp(nLastTime, CTransaction::TX_WORK);
    ...
    if (!cacheAgreementBlock.agreement.IsProofOfWork())
    {
        nNextBlockTime = pCoreProtocol->GetNextBlockTimeStamp(nLastTime, CTransaction::TX_STAKE);
        consParam.nWaitTime = nNextBlockTime - 2 - GetNetTime();
        if (consParam.nWaitTime > 0)
        {
            return false;
