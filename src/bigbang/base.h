...
public:
    ...
    virtual bool FetchSuperNode(std::vector<storage::CSuperNode>& nodes) = 0;
...
public:
    ...
    virtual bool SetForkFilter(const std::vector<uint256>& vFork = std::vector<uint256>(), const std::vector<uint256>& vGroup = std::vector<uint256>()) = 0;    const CForkConfig* ForkConfig()
    {
