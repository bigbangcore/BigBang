...
#define CUSTOM_PREFIX(x) slimguard_##x
#endif

#define CUSTOM_MALLOC(x)  \
    CUSTOM_PREFIX(malloc) \
    (x)
#define CUSTOM_FREE(x)  \
    CUSTOM_PREFIX(free) \
    (x)
#define CUSTOM_REALLOC(x, y) \
    CUSTOM_PREFIX(realloc)   \
    (x, y)
#define CUSTOM_MEMALIGN(x, y) \
    CUSTOM_PREFIX(memalign)   \
    (x, y)
