...
inline void StdDebug(const char* pszName, const char* pszErr)
{
    if (STD_DEBUG)
    {
        std::cout << GetLocalTime() << " [DEBUG] <" << pszName << "> {" << GetThreadName() << "} " << pszErr << std::endl;
    }
}

inline void StdLog(const char* pszName, const char* pszErr)
{
    std::cout << GetLocalTime() << " [INFO] <" << pszName << "> {" << GetThreadName() << "} " << pszErr << std::endl;
}

inline void StdWarn(const char* pszName, const char* pszErr)
{
    std::cout << GetLocalTime() << " [WARN] <" << pszName << "> {" << GetThreadName() << "} " << pszErr << std::endl;
}

inline void StdError(const char* pszName, const char* pszErr)
{
    std::cout << GetLocalTime() << " [ERROR] <" << pszName << "> {" << GetThreadName() << "} " << pszErr << std::endl;
}
