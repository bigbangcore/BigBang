...
void CDeFiProfile::Save(std::vector<unsigned char>& vchProfile) const
{
    ...
    os << nDecayCycle << nCoinbaseDecayPercent << nInitCoinbasePercent << nRewardCycle << nSupplyCycle << nStakeRewardPercent << nPromotionRewardPercent << nStakeMinToken << mapPromotionTokenTimes;
}

void CDeFiProfile::Load(const std::vector<unsigned char>& vchProfile)
{
    CIDataStream is(vchProfile);
    is >> nDecayCycle >> nCoinbaseDecayPercent >> nInitCoinbasePercent >> nRewardCycle >> nSupplyCycle >> nStakeRewardPercent >> nPromotionRewardPercent >> nStakeMinToken >> mapPromotionTokenTimes;
}
