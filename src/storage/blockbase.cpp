...
bool CBlockBase::GetBlockView(const uint256& hash, CBlockView& view, bool fCommitable)
{
...

    {
        ...
        for (CBlockIndex* p = pForkLast; p != pBranch; p = p->pPrev)
        {
            ...
            {
                StdTrace("[BlockBase][TRACE]", "Chain rollback[remove]: remove tx`%s`"
                    , block.vtx[j].GetHash().ToString().c_str());
                view.RemoveTx(block.vtx[j].GetHash(), block.vtx[j], block.vTxContxt[j]);
            }
            if (!block.txMint.IsNull())
            {
                StdTrace("[BlockBase][TRACE]", "Chain rollback[remove]: remove mint tx`%s`"
        ...
        if (nRemoved > 0)
        {
            StdTrace("[BlockBase][TRACE]", "Chain rollback[remove]: remove block amount:%u:", nRemoved);
        }
        ...
        if (vPath.size() > 0)
        {
            StdTrace("[BlockBase][TRACE]", "Chain rollback[add]: added block amount:%u:", vPath.size());
        }
    }
    return true;
}
