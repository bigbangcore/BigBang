...
static const unsigned int CANARY_SIZE = 16U;
...
_unprotected_ptr_from_user_ptr(void* const ptr)
{
    ...
    canary_ptr = ((unsigned char*)ptr) - 16U;
...
static void* my_sodium_malloc(size_t size)
{
    void* user_ptr;
    ...
    // page_zie <= 16 bytes     ||  page_size < sizeof(size_t)
    if (page_size <= sizeof canary || page_size < sizeof unprotected_size)
    {
        sodium_misuse();
    }

    // size_with_canary = 16 bytes + user excatly size
    ...
    canary_ptr = unprotected_ptr + _page_round(size_with_canary) - size_with_canary;
    ...
    memset(user_ptr, 0xdb, size);

    return user_ptr;
}

[[noreturn]] static void _out_of_bounds(void)
{
#ifdef SIGSEGV
    raise(SIGSEGV);
...
static void my_sodium_free(void* ptr)
{
    unsigned char* base_ptr;
    unsigned char* canary_ptr;
    unsigned char* unprotected_ptr;
